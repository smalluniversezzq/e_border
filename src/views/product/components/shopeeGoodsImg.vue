    ...
    <a-card>
      ...
      <template>
        <div class="clearfix">
          ...
          <div v-show='fileList.length < 9' class='upload-box' @mouseover='showUpload' @mouseleave ='hideUpload'>
            <div class='mt-lg'>
              <div>
                <a-icon type="plus" />
              </div>
      ...
      <a-form-item :wrapperCol="{md: {span: 20, offset: 0}, lg: {span: 20, offset: 1}, xl: {span: 20, offset: 1}, xxl: {span: 20, offset: 2}}">
        ...
        <a-button type="primary" style='margin-left:30px; width:100px;' :loading='subloading' @click="updateImgContent">保存</a-button>
      </a-form-item>
    </a-card>
      <!-- 网络图片上传 -->
    <a-modal
  ...
  export default {
    ...
    methods:{
      ...
      toMeitu(picUrl){
        ...
        this.M_URL.pic_URL = ''
        this.M_URL.file_ID = ''
        window.open(this.GLOBAL.BASE_URLS+"beautify/index.html?url="+picUrl+"&index="+this.fileID+'&type='+'1'+'&base_url='+this.GLOBAL.BASE_URL+'&token='+Vue.ls.get("Access-Token"))
