  ...
  <div>
    <div>
      <draggable
       ...
       <div class='upload-box' v-show='this.fileList.length<9' @mouseover='showUpload' @mouseleave ='hideUpload'>
            <div class='mt-lg'>
              <div>
                <a-icon type="plus" />
              </div>
      ...
      <!-- <div v-show='this.fileList.length<9'>
        <a-upload
          :action="this.GLOBAL.BASE_URL+'api/b1/product/upload?token='+this.token"
          :multiple='true'
          :defaultFileList="fileList"
      ...
      <div>
        <a-button v-if='allImg' @click='changePic' :loading='loading'>一键拉伸图片</a-button>
      </div>
      ...
      <a-form-item class='tar' :wrapperCol="{md: {span: 20, offset: 0}, lg: {span: 20, offset: 1}, xl: {span: 20, offset: 1}, xxl: {span: 20, offset: 2}}">
        <a-button v-show='false'  type="primary" style='margin-left:30px; width:100px;' :loading='subloading' @click="updateImgContent">保存</a-button>
      </a-form-item>
           <!-- 网络图片上传 -->
      <a-modal
        title="网络图片上传"
...
<script>
  ...
  import { shopeeGetBase, imgRessize, authCountry, mwsReverse, shopeeUpdateImages, search_category,cutOutPic, transPic } from '@/api/collect'
  export default {
    name:"",
    props:['allImg'],
    components: {
    ...
    methods:{
      ...
      addImage(pic){
        // console.log(pic,'pic')
        this.$emit('shopeeAddVariant',pic)
      },
      handlePreview (file ) {
        ...
        setTimeout(() => {
          ...
          console.log(this.nWidth,this.nHeight,'尺寸')
        }, 300);
      },
      handleChange ({ fileList, file }) {
        this.fileName = file.thumbUrl
        this.fileList = fileList
        // console.log(fileList, file, '222222222222')
      ...
      toMeitu(picUrl){
        ...
        this.M_URL.pic_URL = ''
        this.M_URL.file_ID = ''
        window.open(this.GLOBAL.BASE_URL+"beautify/index.html?url="+picUrl+"&index="+this.fileID+'&type='+'1'+'&base_url='+this.GLOBAL.BASE_URL+'&token='+Vue.ls.get("Access-Token"))
      ...
      updateImgContent(type){
        ...
        console.log(this.goodsimgs,'goodsimgs')
