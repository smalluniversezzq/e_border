  ...
  <div>
    <a-card>
      <a-tabs defaultActiveKey="1" type="card" class="intcardtab" @change='callback'>
        <!-- :disabled='!hasPerm(permsBtn.shopee_product_list)'  -->
        <a-tab-pane  key="1">
          <span slot="tab" >
            产品列表
          </span>
          ...
          <a-card class='mt'>
            <a-row style='margin-bottom:10px'>
              <a-col  :md="20" :sm="20">
                <a-button type="primary"  v-if='hasPerm(permsBtn.shopee_product_exportAll)'  @click='exportListAll()'>批量导出</a-button>
                <a-button type="primary"  v-if='hasPerm(permsBtn.shopee_product_editAll)' style="margin-left:10px;" @click='EditsAll()'>批量编辑</a-button>
                <a-button type="danger"   v-if='hasPerm(permsBtn.shopee_product_delAll)'  style="margin-left:10px;" @click='collectdelAll()'>批量删除</a-button>
              ...
              <a-col :md='4'>
                <a-row type='flex' justify='end'>
                  <a-col>
                    <a-button style="margin-right:20px" type='primary'   v-if='hasPerm(permsBtn.shopee_product_add)'  @click='addProduct'>新增商品</a-button>
                    <a-button type='primary' @click='toReleasedAll'   v-if='hasPerm(permsBtn.shopee_product_joinPublishing)' :loading="addsloading">加入发布</a-button>
                  </a-col>
                </a-row>
              </a-col>
            </a-row>
            <a-row>
              <a-table
                ...
                >
                <template slot-scope="text, record">
                  <div v-for="(item,index) in shopeeCountry" :key='index' v-if='item.code === record.station'>
                    {{item.title}}
                    <div>
                      {{record.status==="0"?"（不可发布）":"（可发布）"}}
                    </div>
                  </div>
                ...
                >
                  <template slot-scope="text, record">
                    <div>
                      <a-row>
                        <a-col :span="12">
                          <a-button type="primary" :disabled='!hasPerm(permsBtn.shopee_release_edit)' @click='Edits(record.id,record.platform)'>编辑</a-button>
                        </a-col>
                        <a-col :span="12">
                          <a-button  :disabled='!hasPerm(permsBtn.shopee_release_del)'  type="danger" style="margin-left:10px;" @click='collectdel(record.id)'>删除</a-button>
                        </a-col>
                      </a-row>
                    </div>
                    <div>
                      <a-row>
                        <a-col :span="12"  >
                          <a-button  :disabled='!hasPerm(permsBtn.shopee_product_joinPublishing)'    class='mt-sm' :loading='addId == record.id ? addloading : false'  @click='released(record.id)'>加入发布</a-button>
                          <!-- <a-button v-if='record.publish == 0'   class='mt-sm' :loading='addId == record.id ? addloading : false'  @click='released(record.id)'>加入发布</a-button>
                          <a-button v-if='record.publish == 1' class='mt-sm' :loading='addloading'>已加入</a-button> -->
                        </a-col>
                      </a-row>
                    </div>
                  </template>
          ...
          <a-modal
            ...
            <a-row class='mt-sm'>
              <a-col span='4' class='tar mr'>请选择站点:</a-col>
              <a-col span='17'>
                <a-select 
                v-model='station'
                 @change="siteChange"
            ...
            <a-row class='mt-sm'>
              <a-col span='4' class='tar mr'>产品分类:</a-col>
              <a-col span='17'>
                <shopeeClass  
                @getAttributeList="getAttributeList" ref='shopeeClassRef'>
                </shopeeClass>
              </a-col>
            </a-row>
            <a-row class='mt-sm'>
              <a-col span='4' class='tar mr'>产品属性:</a-col>
              <a-col span='17'>
            ...
            <a-row class='mt-sm'>
              ...
              <a-col span='17' style='overflow:hidden'>
                <div class='mr flex-min-max ' style='float:left;margin-bottom:5px'>
                  ...
                  <span>{{"("+shopeelanguageCodeOrcountry[initLanguage]+")"}}</span>
                </div>
                <div class='mr flex-min-max ' style='float:left;margin-bottom:5px'  v-for='(item,index) in orilanguage' :key='index' >
                  <a-button
                  ...
                  <span>{{"("+shopeelanguageCodeOrcountry[item]+")"}}</span>
                </div>
              </a-col>
            </a-row>
            <a-row class='mt-sm'>
            ...
            <a-row class='mt-sm'>
              ...
              <a-col span='17'>
                ...
                <a-button
                  ...
                  v-for='(item,index) in shopeeLanguage '
                  :key='index'
                  size="small"
            ...
            <a-form :form="form" class="editAll">
              <a-form-item v-show = 'initCountry == this.initLanguage'>
                <a-row class='mt-sm'>
                  <a-col span='4' class='tar mr'>商品标题:</a-col>
                  <a-col span='17'>
                    <div>
                      <a-input
            ...
            <template slot="footer">
              ...
              <a-button key="submit" type="primary" :loading="editLoading" @click="editOk">
              确定
              </a-button>
            </template>            
          </a-modal>
        </a-tab-pane>
        <a-tab-pane :disabled='!hasPerm(permsBtn.shopee_product_release)' key="2">
          <span slot="tab" >
            产品发布
          </span>
          <shopee-release>
...
<script>
import Vue from 'vue'
import { deleteGoodList, shopeeProductIndex ,country_list, trans_fee , crawler ,productList , productApply ,boxdel ,getcountry,getcategory, getrank , crawlerQueryCollect,exportList ,trans ,getGoodsList ,prodel ,productCopy ,getMyStore ,ExportCheck ,insertPublish , batchUpdate ,getauthCountrys ,authCountry,search_category,changeId,transPlugin , getPersonalCategory , getAmazonCategory ,mwsReverse ,shopeeDeleted ,shopeeBatchUpdate ,shopeeExportList,insertShopee} from '@/api/collect'
import { ACCESS_TOKEN} from '@/store/mutation-types'
import { codeORcountry ,codeORcountrys,shopeeLanguage ,languageCode ,shopeelanguageCodeOrcountry,currencyList, Europe ,shopeeCountry} from '@/views/order/public/country'
...
import shopeeClass from './components/shopeeClass.vue'
import shopeeRelease from './components/shopeeRelease.vue'
...
export default {
  ...
  data () {
    return {
      ...
      // 分类
      initCountry:"",
      DEnum:"",ESnum:"",ITnum:"",USnum:"",CAnum:"",MXnum:"",AEnum:"",BRnum:"",FRnum:"",GBnum:"",INnum:"",TRnum:"",JPnum:"",AUnum:"",PLnum:"",
      selectListDE:[],
      selectListES:[],
      selectListIT:[],
      selectListUS:[],
  ...
  created(){
    ...
    console.log(this.GLOBAL.chrome.id,'id')
  },
  mounted () {
    const downloadFormslUrl=this.GLOBAL.DOWNLOAD_FORMSL_URL
    const baseUrls=this.GLOBAL.BASE_URL
    const Token = Vue.ls.get('Access-Token')
    // console.log(this.$route.query.isBlack,'lll')
    if(this.$route.query.isBlack){
      ...
      this.BlackAndWhiteList(Vue.ls.get('PAGE_SIZE_SHOPEE'))
    }else{
      if(Vue.ls.get('PAGE_SIZE_SHOPEE')){
        this.currentManege = Vue.ls.get('currentManege_shopee')
        // this.getGoodsLists(Vue.ls.get('PAGE_SIZE_SHOPEE'))
        this.BlackAndWhiteList(Vue.ls.get('PAGE_SIZE_SHOPEE'))
      }else{
    ...
    this.getCountryList();
    this.isChromeEB()
    // console.log(Vue.ls.get('CHROME_ID'),'CHROME_ID',Vue.ls.get('PAGE_SIZE_SHOPEE'))
    ...
    if(this.showChromeUrl ===false){console.log("安装了插件")}
    this.initCountry = 'zh'
  ...
  methods: {
    ...
    getSiteValue(code){
      ...
      // console.log(_this.siteArrValue,"siteArrValue")
    },
    ...
    toTranslate(){
      ...
      Object.keys(this.controllerAll).forEach((item,index)=>{
        if(index === 2){
          this.controllerAll.initialAll === true?this.transAll.bullet_point1 = this.form.getFieldValue('bullet_point1.'+this.initLanguage):this.transAll.bullet_point1=''
          this.controllerAll.initialAll === true?this.transAll.bullet_point2 = this.form.getFieldValue('bullet_point2.'+this.initLanguage):this.transAll.bullet_point2=''
          this.controllerAll.initialAll === true?this.transAll.bullet_point3 = this.form.getFieldValue('bullet_point3.'+this.initLanguage):this.transAll.bullet_point3=''
          this.controllerAll.initialAll === true?this.transAll.bullet_point4 = this.form.getFieldValue('bullet_point4.'+this.initLanguage):this.transAll.bullet_point4=''
          this.controllerAll.initialAll === true?this.transAll.bullet_point5 = this.form.getFieldValue('bullet_point5.'+this.initLanguage):this.transAll.bullet_point5=''
        }else if(index === 3){
          this.controllerAll.keywords === true?this.transAll.keywords = this.form.getFieldValue('keywords.'+this.initLanguage):this.transAll.keywords=''
        }
      })
      if(tranId.length>0){
    ...
    chromeToTransAll(sl,tl,q,country,count){
      ...
      }) : (console.log("插件未安裝"),_this.tranLoading  = false,_this.$notification.success({message: '提醒',description: '插件未安装'}))
    },
    tokenFn(a){
      var k = "";
      var b = 406644;
    ...
    onChangeButton(checked) {
      // console.log(this.allCountrysState, 'OVEE')
        ...
        if(this.selectManageAll){
          ...
          boxdel({
            ...
            if(res.code === 0){
              ...
              // console.log(Vue.ls.get('PAGE_SIZE_SHOPEE'),'ovee')
              if(Vue.ls.get('PAGE_SIZE_SHOPEE')){
                // this.getGoodsLists(Vue.ls.get('PAGE_SIZE_SHOPEE'))
                this.BlackAndWhiteList(Vue.ls.get('PAGE_SIZE_SHOPEE'))
              }else{
    ...
    pagechange(page,pagesize){
      ...
      Vue.ls.set('currentManege_shopee',page)
    ...
    onShowSizeChange(current, pageSize) {
      ...
      this.BlackAndWhiteList(pageSize)
      Vue.ls.set('PAGE_SIZE_SHOPEE',pageSize)
      Vue.ls.set('currentManege_shopee',current)
    },
    // 获取国家 和店铺信息
    getMyStores(){
      getMyStore({}).then((res)=>{
    ...
    EXcountryChange(value){
      ...
      // console.log(this.EXcountryValue,"value");
      let str='';
      for(var i = 0 ;i < this.selectManageAll.length ; i++){
        str += (this.selectManageAll[i].id + ',')
      }
    ...
    onCheck(checkedValues){
      ...
      // console.log('countryCheck = ', this.countryCheck)
    ...
    editOk() {
      ...
      console.log(_this.station,_this.classValue,_this.attribute,_this.subvalues.keywords,'lllllllll')
      ...
      if(_this.checkWarning){
        ...
        var editInfo = {
          ...
          title:_this.subvalues.keywords,
        }
        console.log(editInfo)
        _this.shopeeBatchUpdateFn(editInfo)
      }else{
        // 新加坡不做校验
        if(_this.station ==='SG'){
          var editInfo = {
            ...
            this.shopeeBatchUpdateFn(editInfo)
          }else{
            console.log('不允许保存')
    ...
    shopeeBatchUpdateFn(editInfo){
      shopeeBatchUpdate(editInfo).then((res)=>{
        if(res.code === 0){
          this.$notification.success({
            message: '成功',
            description: res.msg,
    ...
    checkAttributeWarningFn(){
        // Object.keys(_this.subAttribute).forEach((item,index,key)=>{
        //   _this.subAttribute[index]=_this.attribute[index]
        //   // _this.subAttribute[index] = _this.attribute[index] 
        // })
        ...
        this.subAttribute.forEach((item,index)=>{
          ...
          // this.subAttribute[index] = this.attribute[index] 
        })
    ...
    // 提交
    subEdit(){
      console.log(this.EditAll,'商品id',this.EditStoreId,'勾选的国家',this.storeId,'店铺',this.brand_name,"品牌",
      ...
      console.log(this.brand_name.length,this.stock.length,this.keywords.length,"信息",this.storeId ,typeof(this.storeId))
      if(this.defaultSlate == 1){
        trans_fee({
    ...
    copyProduct(id){
      ...
      productCopy({
        ...
        if(res.code === 0){
          ...
          }if(Vue.ls.get('currentManege_shopee')){
            this.currentManege = Vue.ls.get('currentManege_shopee')
            this.BlackAndWhiteList(Vue.ls.get('PAGE_SIZE_SHOPEE'))
            this.copyloading = false
          }else{
    ...
    getauthCountrys(val){
      var _this = this
      console.log("siteValue",_this.siteValue,val,"over")
      authCountry({
    ...
    shopSearch(){
      ...
      let pagesize = Vue.ls.get('PAGE_SIZE_SHOPEE') ? Vue.ls.get('PAGE_SIZE_SHOPEE') : 10
      if(this.countryId && this.searchName){
        this.shopSearchInfo = {
          page:this.currentManege,
          page_size:pagesize,
          area:this.countryId,
      ...
      //搜索和国家筛选
      shopeeProductIndex(this.shopSearchInfo).then(res => {
        if(res.code === 0){
          if(res.data.data.length > 0){
            this.data = res.data.data
            this.pageManage = res.pages
    ...
    getCollectListFn(dataInfo){
      this.listLoading = true
      shopeeProductIndex(dataInfo).then(res => {
        if(res.code === 0){
          if(res.data.data.length > 0){
            this.data = res.data.data
            this.pageManage = res.pages
    ...
    classProcessorFn(data){
      if(JSON.stringify(data) === '{}'){
        ...
        Object.keys(data).forEach((item,index,value)=>{
          ...
          reverseObj.category = data[item]
          reverseArr.push(reverseObj)
        })
        // console.log(reverseArr,'reverseArr')
        // console.log(this.plainOptions1,this.plainOptions2,this.plainOptions3,'ovee')
      ...
      reverseCheck(reverseArr,state){
          let _this = this;
          if(state === '0'){
            _this.selectDefaultData = { "DE": [], "ES": [], "IT": [], "US": [], "CA": [], "MX": [], "AE": [], "BR": [], "FR": [], "GB": [], "IN": [], "TR": [], "AU": [], "JP": [], "PL": [] }
              ...
              reverseArr.forEach(function(item){
                switch(item.country){
                  case "DE":
                    ...
                    _this.getmwsReverse(item.country,item.category).then((res)=>{
